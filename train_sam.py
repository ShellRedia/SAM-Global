...
from dataset import Globe_Dataset_SAM
...
class TrainManager:
    def __init__(self, dataset_train, dataset_val):
        ...
        parameters = [args.epochs, args.model_type, args.remark]
        self.record_dir = "results/SAM/{}/{}".format(time_str, "_".join(map(str, parameters)))
            ...
            with torch.no_grad():
                for images, prompt_points, prompt_type, selected_components, sample_ids in dataloader:
                    ...
                    if is_complete:
                        ...
                        label, pred = labels[0][0].cpu().detach(), preds[0][0].cpu().detach()
...
if __name__=="__main__":
    dataset_train, dataset_val  = Globe_Dataset_SAM(True), Globe_Dataset_SAM(False)
    train_manager = TrainManager(dataset_train, dataset_val)
    train_manager.train()